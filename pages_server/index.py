...
class Index:
    ...
    def __addIndex__(self):
        ...
        def index():
            ...
            print(data)
            visited_num = my_blog.query_field_primary_key(1, ['visited_total'])[0]["visited_total"]
            if not session.get('user_level'):
                my_blog.update_data(1, visited_total=str(visited_num+1))
    ...
    def __addRecently__(self):
        ...
        def recently():
            data = my_articles.query_limit(9,
                                           ['article_time', 'article_title', 'post_key'],
                                           ['update_time', 'article_id'])
            print(data)
            return jsonify(data)
